...
app.layout = dbc.Container([ 
    ...
    dbc.Row([
        ...
        dbc.Col([
            ...
            html.Button(id='frame-button', n_clicks=0),
...
###################### Send new intensities to intensity slider
@app.callback([Output('intensity-slider','min'),
               Output('intensity-slider','max')],
              [Input('frame-button','n_clicks')],
              [State('csv-data', 'data'),
               State('frame-slider','value')]
)

def update_selected_frames(n,data,frame_range):
    ...
    if pd.isna(min_int) | pd.isna(max_int):
        my_Intens = dff["Intensity"]
        display(my_Intens)
